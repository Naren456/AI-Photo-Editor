...
// **Fixed Remove Background Function**
const Removebg = () => {
  let file = fileInput.files[0];
  if (!file) {
  ...
  const key = "MxVR21RJNv6ykJ69aCwb4Aea";
  ...
  fetch("https://api.remove.bg/v1.0/removebg", {
    ...
    .then((blob) => {
      // Generate a temporary URL for the processed image
      const imageURL = URL.createObjectURL(blob);

      // Display the result in the preview area
      previewImg.src = imageURL;

      // Clean up old object URLs if needed (optional)
      // Add logic to revoke old URLs if the image changes frequently
    })
    .catch((error) => {
...
removebg.addEventListener("click", Removebg);
